...
giniIndexCalc <- function(expr_ann_data,nIter=1000){
  ...
  giniTable <- data.frame(matrix(nrow = 139, ncol = nIter)); rownames(giniTable) <- known_genes
...
GeneRank_Gini_full <- data.frame(matrix(nrow = 139, ncol = 3))
...
save(GeneRank_Gini_full, file = "GeneRank_Gini_full.RData")
...
save(geneCombinationScore, file = "geneCombinationScore.RData")
...
corplot <- ggplot(geneCombinationScore, aes(x = gene_number, y = avg_corr)) +
  ...
  scale_color_manual(name = "Method",
                     values = c(
                       ...
                       "prb" = "green4",
                       "average" = "black")) +
  theme(legend.position = "bottom") + geom_vline(xintercept=35,linetype="dashed", color = "red")
corplot
...
pval_plot <- ggplot(geneCombinationScore, aes(x = gene_number, y = avg_p)) +
  ...
  scale_color_manual(name = "Method",
                     values = c(
                       ...
                       "prb" = "green4",
                       "average" = "black")) +
  theme(legend.position = "bottom") + geom_hline(yintercept=0.05,linetype="dashed", color = "black")+ geom_vline(xintercept=35,linetype="dashed", color = "red")
pval_plot
