################################################
##CDK4/6 inhibition quiescence programme validaton
#Measure correlation between different quiescence scores and CDK4/6 activity
...
######################################
#Calculate CDK4/6 activity scores:
...
QS$CDK4_CDK6_score <- z_score$z_score


################################################
###work out the correlations between CDK4/6 scores and the 5 quiescence scores (pancancer and in individual cancer types)
...
Corr <- NULL
for (i in Scores) {
  test <- cor.test(QS$CDK4_CDK6_score, QS[[i]])
  p_val <- test$p.value
  corr <- test$estimate
  P_val <- c(P_val, p_val)
  Corr <- c(Corr, corr)
...
for (a in CT) {
  ...
  for (i in Scores) {
    test <- cor.test(test.data$CDK4_CDK6_score, test.data[[i]])
    p_val <- test$p.value
    corr <- test$estimate
    P_val <- c(P_val, p_val)
    Corr <- c(Corr, corr)
...
pdf("CDK4_6_InhibitionQS_ValidationHeatplot.pdf",height = 3,width = 16)
