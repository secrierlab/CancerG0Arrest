...
###################################
###Barplot quiescent cell composition:

setwd("~/Documents/Dormancy_PhD_project_data/scRNA_seq_treatment_response_data/GSE149383/GSE134836_RAW/")


QuiescenceType <- c("CDK Inhibition","Contact Inhibition","MEK Inhibition","Serum Starvation","Spontaneous","Uncertain","CDK Inhibition","Contact Inhibition","MEK Inhibition","Serum Starvation","Spontaneous","Uncertain")
Days <- c(rep(0,6),rep(3,6))
N <- c(530,237,97,105,158,494,1498,557,569,982,771,1043)
...
# Stacked + percent
library(ggpubr)
pdf("barplot_quiescenceType.pdf", height = 5, width = 4)
...
###Fishers exact test:
setwd("~/Documents/Dormancy_PhD_project_data/scRNA_seq_treatment_response_data/GSE149383/GSE134836_RAW/")
load("QuiescenceScores.RData")
QS$Day <- sapply(QS$SampleID, function(x)
  strsplit(x,"_")[[1]][1])
table(QS$Day)
QS <- QS[QS$Day %in% c("D0","D3"),]
table(QS$QuiescenceType)

QuiescenceType <- c("CDK inhibition","Contact inhibition","MEK inhibition","Serum Starvation","Spontaneous Quiescence")
...
for (i in QuiescenceType) {
  
  print(i)
  
  FE_test_df <- QS
  
  FE_test_df$QuiescenceType <- sapply(FE_test_df$QuiescenceType, function(x)
    ifelse(x %in% i,"Yes","No"))
  test_table <- table(FE_test_df$QuiescenceType,FE_test_df$Day)
  print(test_table)
...
FishersResults <- data.frame(QuiescenceType)
